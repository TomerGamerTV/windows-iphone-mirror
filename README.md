...
- Published WPF fake-worker smoke passing for normal startup/restart/second-instance/stop, stream timeout cleanup, worker crash cleanup, same-worker recovery after a stream timeout, missing Apple device support, locked-device guidance, USB trust guidance, and Developer Mode guidance. The isolated installer verifier passes first install, in-place update, bundled-runtime CLI status, silent uninstall, and install-directory removal.
